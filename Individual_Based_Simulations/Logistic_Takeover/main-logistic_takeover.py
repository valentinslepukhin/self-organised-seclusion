...
def calculate_density_square(positions,flag,dx,X,Y,bins,N_in,density,density_temp,
    ...
    #### Do the averaging over 9 squares.
    for ii in range(X):
        for jj in range(Y):
            if ii==0:
    ...
    for n in range(N_in): #prange(N_in):
        ...
        if flag[n]==1:#if particle in domain
            ...
            if dx<positions[n,0]<Lx-dx:
                if True: #beyond the invisible wall which is now always the case
...
def simulation(arguments):
    ...
    if continuation==True:
        ...
        for n in range(N_max): 
            if 0<positions_temp[n,1]<Ly and 0<positions_temp[n,0]<Lx: #means the particle is initiated and not out
                ...
                initial_fraction_advantageous = 0.3
                if positions[ii,0]<Lx*initial_fraction_advantageous:
                    birth_rate[ii] = b
                    lineage[ii] = 1
                else:
                    birth_rate[ii] = b/(1+s)
        ...
        reset_pop_vector=True #to reset simulation time to 0.
        if reset_pop_vector==True:
            total_pop_left=[]
            total_pop_right=[]
            resistant_fraction_left=[]
            resistant_fraction_right=[]
        ### Print out to initial files as they are now for these simulations.
        file_positions = os.path.join(out_dir,'initial.positions.'+str(ID)+'.dat')
        with open(file_positions,'wb') as f:
            pickle.dump(positions,f)
            f.close()
    ...
    for step in range(step0,step0+steps+1):
        ...
        if step%wr_int==0:
            ...
            total_pop.append(N_inside_left+N_inside_right)
            ...
            resistant_fraction_total.append((number_resistant_left+number_resistant_right)/(N_inside_left+N_inside_right))
            ...
            if step%(20*wr_int)==0:
                ...
                with open(file_total_population_right,'wb') as f:
                    ...
                    f.close()
                file_total_population = os.path.join(out_dir,'total_population.'+str(ID)+'.dat')
                with open(file_total_population,'wb') as f:
                    pickle.dump(total_pop,f)
                    f.close()

                #Resistant fraction left and right and total
                file_res_frac_left = os.path.join(out_dir,'resistant_fraction_left.'+str(ID)+'.dat')
                ...
                with open(file_res_frac_right,'wb') as f:
                    ...
                    f.close()
                file_res_frac_total = os.path.join(out_dir,'resistant_fraction_total.'+str(ID)+'.dat')
                with open(file_res_frac_total,'wb') as f:
                    pickle.dump(resistant_fraction_total,f)
                    f.close()
            
            if step%(100*wr_int)==0:        
                ####Update state of the cavity
                #Output positions
                file_positions = os.path.join(out_dir,'positions.'+str(ID)+'.dat')
                with open(file_positions,'wb') as f:
                    pickle.dump(positions,f)
                    f.close()
                #Output flag
                file_flag = os.path.join(out_dir,'flag.'+str(ID)+'.dat')
                with open(file_flag,'wb') as f:
                    pickle.dump(flag,f)
                    f.close()
                #Output lineage
                file_lineage = os.path.join(out_dir,'lineage.'+str(ID)+'.dat')
                with open(file_lineage,'wb') as f:
                    pickle.dump(lineage,f)
                    f.close()
